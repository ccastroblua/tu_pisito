...
# Initializing variables in session state:
if "apartments_df" not in st.session_state:
    st.session_state.apartments_df = pd.DataFrame()
else:
...
confirmation = st.sidebar.button("Let's search for my apartment!")
...
if confirmation:
    if initial_selection == df_option[1] or initial_selection == df_option[2]:
        ...
        print("Calling Idealista API")

        st.session_state.apartments_df = apartments_df


if st.session_state.apartments_df.empty == False:
    ...
    show_df = func.change_column_names(predicted_df[columns])
